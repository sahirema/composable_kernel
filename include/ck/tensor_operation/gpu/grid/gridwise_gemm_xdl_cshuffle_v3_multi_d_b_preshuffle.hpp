          ...
          false> // ForceNaiveLayout
{
    ...
    // KPackPerGroup). When GemmSpec is K-padding flavored, the B-side preshuffled descriptor
    // right-pads K0 to this value so that out-of-range tail reads are zeroed by CK's
    // bounds-checked tensor descriptor.
    __host__ __device__ static auto CalculateBK0ShuffledPadded(index_t K)
    {
        return math::integer_divide_ceil(K, KPerBlock) * KRepeat;
    }
        ...
        index_t N0, index_t N0Pad, index_t K0, index_t K0Pad)
    {
        ...
        if constexpr(n_pad)
        {
            // N-padding path: build a flat (N0, K0, NkSwizzleNumber) descriptor over the
            // physical preshuffled buffer (which holds exactly N0 * K0 * NkSwizzleNumber
            // elements -- AIter's shuffle_weight does NOT physically pad N), right-pad N0 up
            // to N0Pad (and optionally K0 up to K0Pad), then unmerge N0Pad into the
            // (N0Pad/NWave, NWave) layout the blockwise pipe expects. CK's bounds-checked
            // tensor descriptor returns zero for flat-N0 indices >= N0, so the trailing
            // partial N-tile contributes zero to the accumulator. N0Pad is guaranteed to be a
            // multiple of NWave by CalculateBN0ShuffledPadded
